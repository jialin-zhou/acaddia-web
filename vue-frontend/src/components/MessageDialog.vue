<template>
  <el-dialog v-model="dialogVisible" title="通信报文详情" width="900px" @close="onCancel">
    <el-row :gutter="20">
      <!-- Left Column -->
      <el-col :span="8">
        <el-card class="box-card" header="控制字">
          <el-form :model="message.control" label-position="left" label-width="100px" disabled>
            <el-form-item v-for="(val, key) in message.control" :label="key" :key="key">
              <el-input :model-value="val"></el-input>
            </el-form-item>
          </el-form>
        </el-card>
        ...
        <el-card class="box-card" header="状态字">
          <el-form :model="message.status" label-position="left" label-width="100px" disabled>
            <el-form-item label="Status Word 1">
              <el-input v-model="message.status.word1"></el-input>
            </el-form-item>
            <el-form-item label="Status Word 2">
              <el-input v-model="message.status.word2"></el-input>
            </el-form-item>
          </el-form>
          <div class="status-flags">
            <el-checkbox v-for="(val, key) in message.status.flags" :model-value="val" :label="key" :key="key" disabled></el-checkbox>
          </div>
        </el-card>
      </el-col>
      ...
      <el-col :span="8">
        <el-card class="box-card" header="通道数据">
          ...
          <el-table :data="message.channelData" border size="small" height="350">
            <el-table-column type="index" label="Ch" width="50"></el-table-column>
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" @click="onCancel">Return</el-button>
      </span>
    </template>
  </el-dialog>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      message: {
        control: {
          Lenth: 'cccc', LNName: 'cc', DataSetName: 'cc', LDName: 'cccc', SmpCtr: 'cccc'
        },
        rated: {
          'PhsA.Artg': 'cccc', 'PhsA.Vrtg': 'cccc', 'Neut.Artg': 'cccc', tdr: 'cccc'
        },
        status: {
          word1: 'cccc',
          word2: 'cccc',
          flags: {
            '要求检修': false, '试验状态': false, '唤醒时间数据无效': true,
            '适用于插值法': true, '时间同步无效': false, '电流互感器输出类型': false,
            '比例因子': false
          }
        },
